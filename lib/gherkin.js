...
(function(window) {
  ...
  Gherkin.prototype.match = function match(feature, definition) {
    ...
    if (definition.name.constructor === RegExp) {
      ...
      if (args) { // the given regexp seems to fit the feature.name
        ...
        definitionFn = definition.fn; // TODO> how to determinate if definitionFn has the last parametr as done?
      }
    }
    // just define the fn
  ...
  Gherkin.prototype.processScenario = function processScenario(scenario) {
    ...
    for (item in definitions) {
      result = this.match(scenario, definitions[item]);
  ...
  Gherkin.prototype.processFeature = function processFeature(feature) {
    ...
    // run for every (feature)definition
    for (item in definitions) {
      result = this.match(feature, definitions[item]);
      definitionFn = result.definitionFn;
      args = result.args;
